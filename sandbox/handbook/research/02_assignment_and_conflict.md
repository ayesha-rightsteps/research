# Samjhein: docs/research/02_assignment_and_conflict.md
...
- **Cost matrix banayein:** har drone-target jodi ki doori ka square. `C[i][j] = doori²`
...
- **Bachav ke options (preference order):**
  1. Pehle kuch mat karein (DA-MAPPO ne aisa hi kiya) — bas "reassignments per episode"
     count karein
  ...
  4. Har 50 step pe assign karein (DA-MAPPO: sirf 3-5% loss)

### Conflict graph
- **Idea:** har drone ko har doosre se mat jodein. Sirf un jodiyon ko jodein jinki
  **aane wale kuch second mein takkar** ho sakti hai. Drones zyada ho to bhi graph
  chhota rehta hai.
- **CPA maths (theek se karein):** relative position `p` aur relative velocity `v` se —
  sabse kareeb aane ka time `t* = −(p·v)/(v·v)`, usko 0 se H ke beech clip karein.
...
- ⚠️ Guide sirf `t = H` pe doori check karta hai — wo galat hai (jo pehle kareeb aa ke
  door ho jaate hain wo miss ho jaate). Upar wala formula use karein.
...
- **Observation mein neighbors:** conflict neighbors ko `t*` ke order mein sort karein,
  pehle 4 lein, zero se pad karein.
...
- **Hysteresis** = "badalne mein sust" — chhote change pe switch mat karein
